...
# Release Year and Popularity Over Time

Steam has grown significantly over the past two decades, and the platform environment has changed as the number of releases and the player base expanded. To understand these long-term shifts, we examine how the median popularity of games varies by release year.

```{r release_year_trend, message=FALSE, warning=FALSE}
release_trend_plot <- steam_df |>
  ...
  ggplot(aes(
    ...
    text = paste0(
      "Year: ", release_year,
      "<br>Median peak CCU: ", scales::comma(round(median_peak_ccu, 0)),
...
ggplotly(release_trend_plot, tooltip = "text")

```

The trend over time reveals a U-shaped pattern in median peak CCU.  
Games released in the early 2000s show relatively high median popularity because the Steam catalog was much smaller and successful titles dominated player attention.

Between 2005 and 2015, median peak CCU declines steadily. This period corresponds to rapid growth in the number of titles on Steam, increasing competition and making it harder for individual games to stand out.

Starting around 2018, median popularity rises again. This increase may be influenced by a larger global player base, stronger marketing pipelines, and the rise of large-scale multiplayer titles.

Overall, release year alone does not determine a game's success, but the trend highlights structural changes in the Steam ecosystem that affect engagement differently across time.


# Playtime and Engagement

Peak CCU captures how many players are active at the same time, while playtime reflects how long players stay with a game. 
Some titles achieve high peaks but only keep players for a short period, whereas others maintain a smaller but very loyal audience. 
To examine this dimension of engagement, we look at how average playtime relates to peak concurrent users.

```{r playtime_engagement, message=FALSE, warning=FALSE}
playtime_ccu_plot <- steam_df |>
  filter(
    average_playtime > 0,
    peak_ccu > 0
  ) |>
  ...
  labs(
    ...
    x = "Average Playtime (hours, log scale)",
    y = "Peak CCU (log scale)"
  )

ggplotly(playtime_ccu_plot)

```

On the log–log scale, we see a clear positive trend: games that keep players for many hours tend to also reach higher peak CCU. 
However, the point cloud is wide. Some games accumulate long playtimes with only moderate peak CCU, likely niche titles with very dedicated communities. 
Other games spike to high peak CCU with relatively shorter average sessions, such as competitive or seasonal multiplayer titles.

This pattern shows that popularity on Steam is multi-dimensional. 
Concurrent users, playtime, and review sentiment capture different aspects of engagement. 
Including playtime in our later models may help distinguish between short-lived hype and sustained engagement.
