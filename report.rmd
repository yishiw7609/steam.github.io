...
## Additional EDA: Popularity Trends by Release Year

To understand how game popularity evolves over time, we examine how median peak CCU changes across release years.

```{r}
release_trend_plot <- steam_df |>
mutate(
...
summarize(
n_games = n(),
median_peak_ccu = median(peak_ccu, na.rm = TRUE),
.groups = "drop"
) |>
ggplot(aes(
x = release_year,
...
geom_line(color = "#66c0f4") +
geom_point(color = "#66c0f4") +
scale_y_log10(labels = scales::comma) +
scale_x_continuous(breaks = scales::pretty_breaks()) +
labs(
title = "Median Game Popularity by Release Year",
x = "Release Year",
y = "Median Peak CCU (log scale)"
)

ggplotly(release_trend_plot)

```


In particular, we examine not only cross-sectional patterns (e.g., owners, price, reviews), but also temporal trends and engagement-related quantities such as release year and average playtime.
...
In addition to these pairwise correlations, we also examined how popularity evolves across release years and how it relates to playtime. The release-year trend shows a U-shaped pattern in median peak CCU: early titles on Steam enjoy relatively high median popularity, which then declines as the catalog becomes more crowded, and rises again in recent years as the overall player base and large-scale multiplayer releases grow. 

## Additional EDA: Playtime and Player Engagement

We also explore how average playtime relates to peak CCU, using a log–log scale to highlight engagement patterns across games.


```{r}
playtime_peak_plot <- steam_df |>
ggplot(aes(x = average_playtime, y = peak_ccu)) +
geom_point(alpha = 0.2, size = 0.7, color = "#66c0f4") +
scale_x_log10(
labels = label_number(accuracy = 1, big.mark = ","),
breaks = trans_breaks("log10", function(x) 10^x)
) +
...
labs(
title = "Average Playtime vs Peak CCU",
x = "Average Playtime (minutes, log scale)",
y = "Peak CCU (log scale)"
)

ggplotly(playtime_peak_plot)

```

Similarly, our playtime–engagement analysis on a log–log scale indicates a clear positive association between average playtime and peak CCU. Games that keep players for many hours tend to reach higher peak concurrency, although there is substantial variation: some niche titles have modest peaks but very long sessions, while other games achieve high peaks with shorter average playtime. These temporal and engagement patterns support the decision to use flexible, non-linear models that can capture heterogeneous behaviors across different types of games.
