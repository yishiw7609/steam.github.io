...
## Potential Predictors for Popularity

### Estimated Owners vs Popularity 

Owners represent the overall reach of a game while peak_ccu reflects active use. The plot below visualizes the correlation between estimated owners and peak CCU.
...
### Playtime vs Popularity

We also explore how average playtime relates to peak CCU.
...
The median peak CCU is similar for both free-to-play and paid games, indicating that there is no fundamental difference in popularity between typical free-to-play games and typical paid games. Based on the above analysis, we further divide the price of paid games into different ranges to observe the median CCU at different price levels.
...
### Popularity by Developer

Here, we want to examine which developers achieve the highest popularity and user stickness:

```{r}
dev_quality_plot = steam_df |>
  ...
  slice_head(n = 15) |>
  ggplot(aes(y = reorder(developers, median_ccu), x = median_ccu)) +
  geom_col(fill = "#66c0f4") +
  scale_x_log10(labels = scales::comma) +
  labs(
    title = "Top 15 Developers by Peak CCU",
    y = "Developer",
    x = "Median Peak CCU (log scale)"
  )

ggplotly(dev_quality_plot)
```

We notice studios with the high median peak ccu are well-established AAA teams whose success stems from high production value, strong IP franchises, and consistent player engagement. This highlights that sustained quality and brand strength are what drive meaningful player traffic on the platform.
...
The most significant predictor of peak_ccu was the release date. To find how popularity evolves across release years and how it relates to playtime. We exclusively plot their correlation below.
...
The release-year trend shows a U-shaped pattern in median peak CCU: early titles on Steam enjoy relatively high median popularity, which then declines as the catalog becomes more crowded, and rises again in recent years as the overall player base and large-scale multiplayer releases grow.

## Findings

Below are our main findings from this study:

- **Fans of Recency (`release_date`)**: This indicates a strong "recency bias" in the Steam marketplace: people prefer new stuff. Newer games benefit from the "hype cycle" and marketing pushes, while older titles—regardless of quality—tend to see significant player churn. The model successfully learned the lifecycle decay of a typical video game.
